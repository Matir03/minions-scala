...
/** The second layer of the board implementation. See BoardState.scala for the
  * more primitive layer below. Actions and semantics on this layer are more at
  * the level of the UI, rather than the direct legal actions in the game. In
  * particular, this layer implements undo/redo and action reordering that make
  * the UI friendly:
  *
  *   - Spawn PlayerActions are reordered to happen after all other
  *     PlayerActions. This allows us to legality-check everything AS IF all
  *     spawns happened at the end of the turn in a distinct "spawn phase" as
  *     required by the technical rules of minions, and yet permits us to have a
  *     UI that in practice allows users to interleave spawns and other actions.
  *
  *   - Local undos allow a user to specify a piece and undo all actions
  *     involving that piece this turn. We do this simply by stripping out all
  *     actions involving the piece, and legality-checking the remaining
  *     sequence of actions, tossing out any that don't legality check.
  *
  * In general, we deal with any dependency conflicts from reordering of actions
  * by throwing out actions that became illegal.
  *
  * As a slight detail, GeneralBoardActions require communication with the
  * outside world when doing them or attempting to undo or redo them. Doing or
  * redoing requires claiming a shared resource (like spending souls to buy a
  * unit), undoing requires informing the broader game the resource is available
  * again (like regaining the souls after undoing the purchase). We handle this
  * by exposing functions here to allow users to determine what is about to get
  * undone or redone on an undo or redo, namely prevAction and nextAction. It's
  * up to the user of Board to do the necessary work here.
  */

/** BoardAction: A single UI/user action that can be made on a board. These are
  * the objects that conceptually form a stack that global undo/redo operates on
  * \- yes, this means that local undos are treated the same way as a group of
  * PlayerActions from the UI perspective - and the local undo itself can be
  * undone/redone. The actionId values are simply strings intended to
  * disambiguate different actions, so that when we merge and dedup action
  * sequences we can tell which actions were actually distinct.
  */
sealed trait BoardAction
case class PlayerActions(actions: List[PlayerAction], actionId: String)
    extends BoardAction
case class DoGeneralBoardAction(action: GeneralBoardAction, actionId: String)
    extends BoardAction

case class Redo(actionId: String) extends BoardAction
//Undo the most recent List[PlayerAction] involving this piece.
case class LocalPieceUndo(pieceSpec: PieceSpec, actionId: String)
    extends BoardAction
...
case class BuyReinforcementUndo(pieceName: PieceName, actionId: String)
    extends BoardAction
...
case class Board private (
    ...
    private var playerGeneralBoardActionsPrevTurns: Vector[
      (Vector[List[PlayerAction]], Vector[GeneralBoardAction])
    ]
) {
  ...
  def findLocalPieceUndoActions(
      pieceSpec: PieceSpec
  ): Option[List[PlayerAction]] = {
    findLastMatch(history.spawnActionsThisTurn) { playerActions =>
      playerActions.exists { _.involvesPiece(pieceSpec) }
    } match {
      case Some(actions) => Some(actions)
      case None =>
        findLastMatch(history.moveAttackActionsThisTurn) { playerActions =>
          playerActions.exists { _.involvesPiece(pieceSpec) }
        }
    }
  }
  ...
  def findSpellUndoActions(spellId: SpellId): Option[List[PlayerAction]] = {
    findLastMatch(history.spawnActionsThisTurn) { playerActions =>
      playerActions.exists { _.involvesSpell(spellId) }
    } match {
      case Some(actions) => Some(actions)
      case None =>
        findLastMatch(history.moveAttackActionsThisTurn) { playerActions =>
  ...
  def findGainSpellUndoAction(spellId: SpellId): Option[GeneralBoardAction] = {
    findLastMatch(history.generalBoardActionsThisTurn) { generalBoardAction =>
  ...
  def findBuyReinforcementUndoAction(
      pieceName: PieceName
  ): Option[GeneralBoardAction] = {
    findLastMatch(history.generalBoardActionsThisTurn) { generalBoardAction =>
  ...
  def tryLegality(
    ...
    action match {
      case PlayerActions(actions, _) =>
        curState().tryLegality(actions, externalInfo)
      case DoGeneralBoardAction(action, _) =>
        curState().tryGeneralLegality(action, externalInfo)
      case Redo(_) =>
      ...
      case LocalPieceUndo(pieceSpec, _) =>
        val anyActionInvolvesPiece = {
          history.moveAttackActionsThisTurn.exists { playerActions =>
            playerActions.exists { _.involvesPiece(pieceSpec) }
          } ||
          history.spawnActionsThisTurn.exists { playerActions =>
            playerActions.exists { _.involvesPiece(pieceSpec) }
          }
        }
      ...
      case SpellUndo(spellId, _) =>
        ...
        if (!anyActionInvolvesSpell)
          Failure(
            new Exception("No actions to undo involving this spell this turn")
          )
        else
          Success(())
      ...
      case BuyReinforcementUndo(pieceName, _) =>
        if (!externalInfo.pieceMap.contains(pieceName))
          Failure(
            new Exception(
              "Trying to undo buying reinforcement piece with unknown name: " + pieceName
            )
          )
        else if (
          !history.generalBoardActionsThisTurn.exists { generalAction =>
            generalAction.involvesBuyPiece(pieceName)
          }
        )
          Failure(
            new Exception(
              "Cannot undo buying a piece that was not bought this turn"
            )
          )
        else
          Success(())
      ...
      case GainSpellUndo(spellId, _) =>
        if (
          !history.generalBoardActionsThisTurn.exists { generalAction =>
            generalAction.involvesGainSpell(spellId)
          }
        )
          Failure(
            new Exception("No actions to undo involving this spell this turn")
  ...
  def addSpecsToFilter(
      pieceSpecsToFilter: Set[PieceSpec],
      actions: List[PlayerAction]
  ): Set[PieceSpec] = {
    var specs = pieceSpecsToFilter
    actions.foreach { action =>
      specs = specs ++ action.additionalUndoPieceSpecs()
    }
    specs
  }
  ...
  def undoLastMatch(
      externalInfo: ExternalInfo,
      matchesPlayer: List[PlayerAction] => Boolean,
      matchesGeneral: GeneralBoardAction => Boolean
  ): BoardHistory = {
    val spawnMatchIdx =
      history.spawnActionsThisTurn.lastIndexWhere(matchesPlayer)
    if (spawnMatchIdx >= 0) {
      ...
      val preSpawnActionsThisTurn =
        history.spawnActionsThisTurn.take(spawnMatchIdx)
      preSpawnActionsThisTurn.foreach { playerActions =>
        newSpawnState.doActions(playerActions, externalInfo)
      }
      var pieceSpecsToFilter: Set[PieceSpec] = Set()
      pieceSpecsToFilter = addSpecsToFilter(
        pieceSpecsToFilter,
        history.spawnActionsThisTurn(spawnMatchIdx)
      )
      val postSpawnActionsThisTurn =
        reapplyLegal(
          history.spawnActionsThisTurn.drop(spawnMatchIdx + 1),
          { playerActions =>
            newSpawnState.doActions(playerActions, externalInfo)
          },
          { playerActions =>
            playerActions.exists { action =>
              pieceSpecsToFilter.exists { spec => action.involvesPiece(spec) }
            }
          },
          { actions =>
            pieceSpecsToFilter = addSpecsToFilter(pieceSpecsToFilter, actions)
          }
        )
      val newSpawnActionsThisTurn =
        preSpawnActionsThisTurn ++ postSpawnActionsThisTurn
      BoardHistory(
      ...
      val moveAttackMatchIdx =
        history.moveAttackActionsThisTurn.lastIndexWhere(matchesPlayer)
      if (moveAttackMatchIdx >= 0) {
        val newMoveAttackState = initialStateThisTurn.copy()
        val newGeneralBoardActionsThisTurn = history.generalBoardActionsThisTurn
        newGeneralBoardActionsThisTurn.foreach {
          newMoveAttackState.doGeneralBoardAction(_, externalInfo)
        }
        val preMoveAttackActionsThisTurn =
          history.moveAttackActionsThisTurn.take(moveAttackMatchIdx)
        preMoveAttackActionsThisTurn.foreach { playerActions =>
          newMoveAttackState.doActions(playerActions, externalInfo)
        }
        var pieceSpecsToFilter: Set[PieceSpec] = Set()
        pieceSpecsToFilter = addSpecsToFilter(
          pieceSpecsToFilter,
          history.moveAttackActionsThisTurn(moveAttackMatchIdx)
        )
        val postMoveAttackActionsThisTurn =
          reapplyLegal(
            history.moveAttackActionsThisTurn.drop(moveAttackMatchIdx + 1),
            { playerActions =>
              newMoveAttackState.doActions(playerActions, externalInfo)
            },
            { playerActions =>
        ...
        val newMoveAttackActionsThisTurn =
          preMoveAttackActionsThisTurn ++ postMoveAttackActionsThisTurn
        val newSpawnState = newMoveAttackState.copy()
        val newSpawnActionsThisTurn =
          reapplyLegal(
            history.spawnActionsThisTurn.drop(spawnMatchIdx + 1),
        ...
        val newMoveAttackState = initialStateThisTurn.copy()
        val newGeneralBoardActionsThisTurn =
          dropLastMatch(history.generalBoardActionsThisTurn)(matchesGeneral)
        newGeneralBoardActionsThisTurn.foreach {
          newMoveAttackState.doGeneralBoardAction(_, externalInfo)
        }
        var pieceSpecsToFilter: Set[PieceSpec] = Set()
          ...
          val newHistory = action match {
            case PlayerActions(playerActions, _) =>
                ...
                playerAction match {
                  ...
                  case ActivateTile(loc) =>
                    newMoveAttackState.tiles(loc).terrain match {
                      case Wall | Ground | Water(_) | Graveyard | SorceryNode |
                          Teleporter | Mist | Earthquake(_) | Firestorm(_) |
                          Whirlwind(_) =>
                        assertUnreachable()
                      case Spawner(_) =>
                        delayedToSpawnRev = playerAction :: delayedToSpawnRev
                  ...
                  case PlaySpell(spellId, _) =>
                    ...
                    if (spell.spawnPhaseOnly)
                      ...
                      newMoveAttackState.doAction(
                        playerAction,
                        externalInfo
                      ) match {
                        case Success(()) =>
                          moveAttackActionsRev =
                            playerAction :: moveAttackActionsRev
                        case Failure(_) =>
                          delayedToSpawnRev = playerAction :: delayedToSpawnRev
                      }
                    }
              ...
              val reappliedSpawnActionsThisTurn =
                reapplyLegal(
                  history.spawnActionsThisTurn,
                  { playerActions =>
                    newSpawnState.doActions(playerActions, externalInfo)
                  },
                  { playerActions =>
                    playerActions.exists { action =>
                      pieceSpecsToFilter.exists { spec =>
                        action.involvesPiece(spec)
                      }
                    }
                  },
                  { actions =>
                    pieceSpecsToFilter =
                      addSpecsToFilter(pieceSpecsToFilter, actions)
                  }
                )
              ...
              BoardHistory(
                ...
                spawnState = newSpawnState,
                redoStack = None,
                moveAttackActionsThisTurn =
                  history.moveAttackActionsThisTurn :+ moveAttackActionsRev.reverse,
                spawnActionsThisTurn =
                  reappliedSpawnActionsThisTurn :+ spawnActions,
                generalBoardActionsThisTurn =
            ...
            case DoGeneralBoardAction(generalBoardAction, _) =>
              ...
              newMoveAttackState.doGeneralBoardAction(
                generalBoardAction,
                externalInfo
              )
              newSpawnState.doGeneralBoardAction(
                generalBoardAction,
                externalInfo
              )
              BoardHistory(
                ...
                generalBoardActionsThisTurn =
                  history.generalBoardActionsThisTurn :+ generalBoardAction
              )
  ...
  def endTurn(externalInfo: ExternalInfo): Unit = {
    ...
    playerGeneralBoardActionsPrevTurns =
      playerGeneralBoardActionsPrevTurns :+ playerGeneralBoardActions
  }
