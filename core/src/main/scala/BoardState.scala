...
/** The board state and various (often mutable) data structures and types
  * relating directly to it, for a single board.
  *
  * This is the base layer of the board implementation - interacting with the
  * board is done via PlayerAction, which represents a basic action performable
  * in the game according to the rules. Note that the model presented here in
  * BoardState is that move/attack/spawn actions are arbitrarily interleavable.
  * Enforcing of a separation of spawn actions into a separate phase at the end
  * of the turn is done by action reordering in Board.scala, the next layer up.
  * This allows the user to input actions in arbitrary order, yet have the
  * result be consistent with having a spawn phase.
  *
  * Also implemented are GeneralBoardActions - gaining spells and buying pieces.
  * These are recorded separately from PlayerActions. This is because they
  * aren't subject to the same undo/redo/reordering logic that player actions
  * are subject to, since they involve interaction with the broader game.
  *
  * Given generalBoard action G and player action P, an invariant that should
  * hold is:
  *   - If P followed by G is legal, then G followed by P must also be legal and
  *     lead to the same board state.
  *   - Also generalBoardActions commute with each other.
  *
  * See Board.scala for the next layer up in the board implementation stack.
  */

/** PieceSpec: Identifies a single piece on the board uniquely on a given turn,
  * and in a way that should be robust to reordering of actions within that
  * turn.
  */
...
case class SpawnedThisTurn(pieceName: PieceName, spawnLoc: Loc, nthAtLoc: Int)
    extends PieceSpec
...
/** PlayerAction: A single action taken by a player. Does not include
  * "GeneralBoard" actions of gaining spells or buying pieces. PlayerActions are
  * immutable and their data should be independent of any of the mutable types
  * on the board. That is, it should be meaningful to ask whether an action
  * would be legal on a different board, or if the order of actions were
  * rearranged, etc.
  *
  * Notes: Movements: movements is a list because we need to support piece
  * swaps, triangular rotations, etc.
  */
sealed trait PlayerAction {
    ...
    this match {
      ...
      case Spawn(spawnLoc, spawnName) =>
        pieceSpec match {
          ...
          case SpawnedThisTurn(pieceName, sLoc, _) =>
            pieceName == spawnName && sLoc == spawnLoc
        }
  ...
  def additionalUndoPieceSpecs(): List[PieceSpec] = {
    this match {
      case Movements(movements) =>
        movements.map { case Movement(pSpec, _) => pSpec }
...
case class Attack(attackerSpec: PieceSpec, targetSpec: PieceSpec)
...
case class ActivateAbility(
    spec: PieceSpec,
    ability: PieceAbility,
    targets: SpellOrAbilityTargets
) extends PlayerAction
case class Blink(pieceSpec: PieceSpec, src: Loc) extends PlayerAction
case class Teleport(pieceSpec: PieceSpec, src: Loc, dest: Loc)
    extends PlayerAction
case class PlaySpell(spellId: SpellId, targets: SpellOrAbilityTargets)
    extends PlayerAction
...
object SpellOrAbilityTargets {
  val none = new SpellOrAbilityTargets(
    PieceSpec.none,
    PieceSpec.none,
    Loc(-1, -1),
    Loc(-1, -1),
  ...
  def terrainAndLoc(terrain: Terrain, loc: Loc) = new SpellOrAbilityTargets(
    PieceSpec.none,
    PieceSpec.none,
    loc,
    Loc(-1, -1),
...
/** GeneralBoardAction: Actions relating to this board that involve interaction
  * with the broader game (a shared spell pool, a shared souls pool). These are
  * NOT part of the normal action stack.
  *
  * Requirement: spellId should be a unique identifier for a particular spell
  * card. Users of BoardState should ensure that this is the case.
  */
...
case class BuyReinforcement(pieceName: PieceName, free: Boolean)
    extends GeneralBoardAction
case class GainSpell(spellId: SpellId) extends GeneralBoardAction

/** Tile: A single tile on the board Possibly enchanted due to spells. Later in
  * list -> spells were played later.
  */
object Tile {
...
/** Piece: MUTABLE - be aware when copying, as we rely sharing of mutation to
  * fields between different references to a piece A single piece on the board.
  */
object Piece {
...
case class Piece private (
  ...
  def curStats(board: BoardState): PieceStats = {
    (modsWithDuration ++ board.tiles(loc).modsWithDuration)
      .foldLeft(baseStats) { (pieceStats, mod) =>
        mod.mod(pieceStats)
      }
  }
...
object BoardState {
  def create(terrain: Plane[Terrain], startLocs: SideArray[Loc]): BoardState = {
    val board = new BoardState(
      ...
      startLocs = startLocs,
      pieces =
        Plane.create(terrain.xSize, terrain.ySize, terrain.topology, Nil),
      pieceById = Map(),
      nextPieceId = 0,
...
object BoardStateOfFragments {
  def ofFragments(
      f0: BoardStateFragment0,
      f1: BoardStateFragment1
  ): BoardState = {
    BoardState(
      tiles = f0.tiles,
...
case class BoardState private (
  ...
  def copy(): BoardState = {
    ...
    val newPiecesSpawnedThisTurn = piecesSpawnedThisTurn.transform {
      (_k, piece) => newPieceById(piece.id)
    }
    newBoard.pieceById = newPieceById
    newBoard.piecesSpawnedThisTurn = newPiecesSpawnedThisTurn
    newBoard.pieces.transform { pieceList =>
      pieceList.map { piece => newPieceById(piece.id) }
    }
    newBoard
  }
    ...
    list match {
      ...
      case _ =>
        tryLegality(actions, externalInfo) match {
          ...
          case Success(()) =>
            list.foreach { action =>
              doActionUnsafeAssumeLegal(action, externalInfo)
            }
            Success(())
        }
    }
  ...
  def resetMana(): Unit = {
    ...
    tiles.foreachi { case (loc, tile) =>
      val occupied = pieceById.values.exists { piece =>
        piece.side == side && piece.loc == loc
      }
  ...
  def spellsToAutoDiscardBeforeEndTurn(
      externalInfo: ExternalInfo
  ): List[SpellId] = {
    var manaTmp = mana
    var discardCount = 0
  ...
  def endTurn(externalInfo: ExternalInfo): Unit = {
    ...
    if (resetState != JustEnded) {
      ...
      pieceById.values.foreach { piece =>
        if (piece.side == side) {
          piece.modsWithDuration =
            piece.modsWithDuration.filter(x => x.mod != PieceMods.NecroPick)
        }
      }
    }
    ...
    Side.foreach { side =>
      ...
      pieceById.values.foreach { piece =>
        if (piece.side == side && !piece.baseStats.isNecromancer)
          allowedFreeBuyPieces(side) =
            allowedFreeBuyPieces(side) + piece.baseStats.name
      }
        ...
        spawnPieceInitial(side, necroStats, startLoc) match {
          ...
          case Success(piece) =>
            if (allowedNecros(side).size > 1) {
              val necroPick =
                PieceModWithDuration(PieceMods.NecroPick, turnsLeft = None)
  ...
  def tryGeneralLegality(
      action: GeneralBoardAction,
      externalInfo: ExternalInfo
  ): Try[Unit] = {
    action match {
      case BuyReinforcement(pieceName, free) =>
        if (!externalInfo.pieceMap.contains(pieceName))
          Failure(
            new Exception(
              "Bought reinforcement piece with unknown name: " + pieceName
            )
          )
        else if (free && !canFreeBuyPiece(side, pieceName))
          Failure(
            new Exception(
              "Cannot buy/carry-over this piece for free: " + pieceName
            )
          )
        else
          Success(())
  ...
  def canFreeBuyPiece(side: Side, pieceName: String): Boolean = {
    allowedFreeBuyPieces(side).contains(pieceName) && numFreeBuysAllowed(
      side
    ) > 0
  }
  ...
  private def forEachLegalMoveHelper(
      piece: Piece,
      pathBias: List[Loc],
      isRotationPath: Boolean
        ...
        if (
          ...
          canShuffleOnPath(
            ...
            case None =>
              ...
              val otherPiecesAbleToFollow = pieces(piece.loc).filter {
                otherPiece =>
                  otherPiece.spec != pieceSpec &&
                  canMoveAtLeast(otherPiece, shortestRevPath.length - 1) &&
                  shortestRevPath.forall { pathLoc =>
                    canWalkOnTile(
                      otherPiece.baseStats,
                      otherPiece.curStats(this),
                      tiles(pathLoc)
                    )
                  }
              }
              ...
              for (numOthers <- 1 to otherPiecesAbleToFollow.length) {
                if (!success) {
                  ...
                  canShuffleOnPath(
                    ...
                    revPath,
                    numMovingFromZero = 1 + numOthers
                  ) match {
                    case Some(shuffles) =>
                      val otherMoves = others.map { other =>
                        (other.spec, shortestRevPath)
                      }
                      f(
                        loc,
                        revPath,
                        (pieceSpec, shortestRevPath) :: (otherMoves ++ shuffles)
                      )
                      endedOn += loc
                      success = true
                    case None => ()
        ...
        if (!seen.contains(loc)) {
          ...
          if (d < range) {
            topology.forEachAdj(loc) { y =>
              ...
              if (
                tiles.inBounds(y) && !revPath
                  .contains(y) && canMoveThroughLoc(piece, y)
              )
                q = q :+ ((y, d + 1, y :: revPath))
            }
          }
  ...
  def legalMoves(piece: Piece): Map[Loc, Int] = {
    ...
    forEachLegalMoveHelper(piece, List(), isRotationPath = false) {
      case (loc, revPath, revMovements) =>
  ...
  def withinTerrainRange(piece: Piece, steps: Int): Set[Loc] = {
    ...
    tiles.topology.forEachReachable(piece.loc) { (loc, dist) =>
      if (
        dist <= steps && inBounds(loc) && canWalkOnTile(
          piece.baseStats,
          piece.curStats(this),
          tiles(loc)
        )
  ...
  def attackableHexes(piece: Piece): Set[Loc] = {
    ...
    val moveLocsPieceCouldAttackFrom = {
      if (stats.isLumbering) Set(piece.loc) else moveLocs
    }
  ...
  def countGraveyards(side: Side): Int = {
    ...
    tiles.foreachi { case (loc, tile) =>
      if (
        tile.terrain == Graveyard && pieceById.values.exists { piece =>
          piece.side == side && piece.loc == loc
        }
      ) {
        ans += 1
  ...
  def findPathForUI(piece: Piece, pathBias: List[Loc], isRotationPath: Boolean)(
      f: (Loc, List[(PieceSpec, List[Loc])]) => Boolean
  ): Option[(List[Loc], List[(PieceSpec, List[Loc])])] = {
    forEachLegalMoveHelper(piece, pathBias, isRotationPath) {
      case (loc, revPath, revMovements) =>
        if (f(loc, revMovements)) {
          val movements = revMovements.map { case (spec, rpath) =>
            (spec, rpath.reverse)
          }
  ...
  def canAttack(
      attackerStats: PieceStats,
      attackerHasMoved: Boolean,
      attackerState: ActState,
      targetStats: PieceStats
  ): Boolean = {
    tryCanAttack(
      attackerStats,
      attackerHasMoved,
  ...
  def tryCanAttack(
    ...
    val attackRange = {
      if (targetStats.isFlying) attackerStats.attackRangeVsFlying
      else attackerStats.attackRange
    }
    if (attackerStats.attackEffect.isEmpty || attackerStats.numAttacks == 0)
      failed("Piece cannot attack")
    else if (attackRange <= 0) failed("Piece cannot attack this unit")
    else if (attackerStats.isLumbering && attackerHasMoved)
      failed("Lumbering pieces cannot both move and attack on the same turn")
    else {
      val result = attackerState match {
        case Moving(_)             => Success(())
        case Attacking(numAttacks) =>
          // Slightly more specific err message
          if (attackerStats.numAttacks == 1 && numAttacks > 0)
            failed("Piece already attacked")
          // Fuller check
          else if (numAttacks >= attackerStats.numAttacks)
            failed("Piece already assigned all of its attacks")
          else Success(())
      ...
      result.flatMap { case () =>
        val result2 = attackerStats.attackEffect match {
          ...
          case Some(TransformInto(_)) =>
            if (targetStats.isNecromancer)
              failed("Necromancers cannot be transformed")
            else Success(())
        }
        result2.flatMap { case () =>
          if (!attackerStats.canHurtNecromancer && targetStats.isNecromancer)
            failed("Piece cannot hurt necromancer")
          else Success(())
        }
      }
    }
  ...
  private def tryCanWalkOnTile(
    ...
    tile.terrain match {
      ...
      case Ground | Graveyard | SorceryNode | Teleporter | Spawner(_) | Mist =>
        Success(())
      case Water(_) =>
        if (pieceStats.isFlying) Success(())
        else failed("Non-flying pieces cannot move or spawn on water")
      case Earthquake(_) =>
        if (baseStats.moveRange >= 2) Success(())
        else
          failed(
            "Only unit types with at least two speed can move through an earthquake"
          )
      case Firestorm(_) =>
        if (baseStats.defense.getOrElse(4) >= 4) Success(())
        else
          failed(
            "Only unit types with at least four health can move through a firestorm"
          )
      case Whirlwind(_) =>
        if (baseStats.isPersistent) Success(())
        else failed("Only persistent unit types can move through a whirlwind")
    }
  }
  ...
  private def canMoveThroughLoc(piece: Piece, loc: Loc): Boolean = {
    ...
    (pieceStats.isFlying || pieces(loc).forall { other =>
      other.side == piece.side
    })
  }
  private def tryCanMoveThroughLoc(piece: Piece, loc: Loc): Try[Unit] = {
    val pieceStats = piece.curStats(this)
    tryCanWalkOnTile(piece.baseStats, pieceStats, tiles(loc)).flatMap {
      case () =>
        if (
          pieceStats.isFlying || pieces(loc).forall { other =>
            other.side == piece.side
  ...
  def canSwarmTogether(
      pieceStats: PieceStats,
      otherStats: PieceStats
  ): Boolean = {
    pieceStats.swarmMax > 1 && otherStats.swarmMax > 1 && pieceStats.name == otherStats.name
  }

  private def canSwarmToLoc(
      pieceSpec: PieceSpec,
      pieceStats: PieceStats,
      loc: Loc,
      simultaneousMovements: List[Movement]
  ): Boolean = {
    ...
    val ok = {
      pieces(loc).forall { other =>
        if (other.spec == pieceSpec) true
          ...
          simultaneousMovements.exists { case Movement(spec, _) =>
            other.spec == spec
          }
        ) true
        ...
        if (pieceSpec == otherSpec) true
          ...
          pieceIfMovingTo(otherSpec, otherPath, loc).forall { other =>
            okSwarmer(other.curStats(this))
          }
      }
    }
  ...
  def tryCanEndOnLoc(
    ...
    if (!tiles.inBounds(loc)) failed("Location not in bounds")
      ...
      tryCanWalkOnTile(baseStats, pieceStats, tiles(loc)).flatMap { case () =>
        if (!pieces(loc).forall { other => other.side == side })
          failed("Piece would end in the same space as enemies")
        else {
          if (
            !canSwarmToLoc(pieceSpec, pieceStats, loc, simultaneousMovements)
          ) {
            if (pieceStats.swarmMax > 1)
              failed(
                "Pieces cannot swarm together or too many in the same space"
              )
  ...
  private def canMoveAtLeast(piece: Piece, steps: Int): Boolean = {
    piece.actState match {
      case Moving(stepsUsed) =>
        piece.curStats(this).moveRange >= stepsUsed + steps
  ...
  private def canShuffleOnPath(
      side: Side,
      pieceStatss: List[PieceStats],
      revPath: List[Loc],
      numMovingFromZero: Int
  ): Option[List[(PieceSpec, List[Loc])]] = {
    ...
    if (
      pieces(
        loc
      ).isEmpty || (remainingPath.length == 0 && numMovingFromZero == pieces(
        loc
      ).length)
    )
      Some(List())
      ...
      val excess = {
        ...
        if (
          !pieceStatss.forall { pieceStats =>
            pieces(loc).forall { otherStats =>
              canSwarmTogether(otherStats.curStats(this), pieceStats)
            }
          }
        )
          pieces(loc).length
      ...
      // Otherwise, if that's the end of the path, then we're stuck.
        ...
        if (!pieces(nextLoc).forall { other => other.side == side })
          ...
          val piecesAbleToShuffle = pieces(loc).filter { piece =>
            canMoveAtLeast(piece, 1) && canWalkOnTile(
              piece.baseStats,
              piece.curStats(this),
              tiles(nextLoc)
            )
          ...
          if (piecesAbleToShuffle.length < excess)
            ...
            canShuffleOnPath(
              ...
              case None => None
              case Some(shuffles) =>
                Some(piecesMoving.map { piece =>
                  (piece.spec, List(nextLoc, loc))
                } ++ shuffles)
            }
          }
        }
  ...
  private def isSpawnerInRange(
      spawnLoc: Loc,
      spawnStats: PieceStats
  ): Boolean = {
    pieceById.values.exists { piece =>
      ...
      if (piece.side != side) false
        ...
        if (
          (!spawnStats.isEldritch || distance > 1) && (spawnerStats.spawnRange
            .forall { d => d < distance })
        ) false
  ...
  def canMoveTerrain(loc: Loc): Try[Unit] = {
    if (pieces(loc).nonEmpty)
      Failure(new Exception("Target location is not empty"))
    else if (tiles(loc).terrain != Ground)
      Failure(new Exception("Target terrain is not Ground"))
    else {
      val pieceInRange =
        pieceById.values.exists { piece =>
  ...
  def moveTerrain(terrain: Terrain, loc: Loc) = {
    tiles.transform { tile =>
      if (tile.terrain == terrain) {
        Tile(
          Ground,
          tile.startingTerrain,
          modsWithDuration = tile.modsWithDuration
        )
      } else {
        tile
      }
    ...
    tiles(loc) = Tile(
      terrain,
      tiles(loc).startingTerrain,
      modsWithDuration = tiles(loc).modsWithDuration
    )
  }

  private def killAttackingWailingUnits(
      externalInfo: ExternalInfo,
      otherThan: Option[PieceSpec] = None
  ): Unit = {
    val attackedWailings = pieceById.iterator.filter { case (_, piece) =>
      piece.curStats(this).isWailing && piece.hasAttacked && otherThan.forall {
        spec => piece.spec != spec
      }
    }
    attackedWailings.toList.foreach { case (_, piece) =>
      killPiece(piece, externalInfo)
    }
  }

  private def trySpellTargetLegality(
      spell: Spell,
      targets: SpellOrAbilityTargets
  ): Try[Unit] = {
    spell match {
      ...
      case (spell: TileSpell) =>
        if (!tiles.inBounds(targets.loc0))
          failed("Target location not in bounds")
  ...
  private def manaInHand(
      side: Side,
      externalInfo: ExternalInfo,
      excludingSpell: Option[SpellId] = None
  ): Int = {
  ...
  private def tryLegalitySingle(
    ...
    action match {
      case Movements(movements) =>
        ...
        val pieceIdsMoved = movements.map { case Movement(pieceId, _) =>
          pieceId
        }
        failIf(
          pieceIdsMoved.distinct.length != movements.length,
          "More than one movement for the same piece"
        )
        ...
        movements.foreach { case Movement(pieceSpec, path) =>
          ...
          failUnless(
            path.forall { loc => tiles.inBounds(loc) },
            "Movement out of bounds"
          )
          failUnless(
            (1 to (path.length - 1)).forall { idx =>
              topology.distance(path(idx - 1), path(idx)) == 1
            },
          ...
          findPiece(pieceSpec) match {
            ...
            case Some(piece) =>
              ...
              piece.actState match {
                case Moving(stepsUsed) =>
                  failUnless(
                    path.length - 1 <= pieceStats.moveRange - stepsUsed,
                    "Movement range of piece is not large enough"
                  )
              ...
              failUnless(
                piece.loc == path(0),
                "Moved piece is not at the head of the path"
              )
      ...
      case Attack(attackerSpec, targetSpec) =>
        (findPiece(attackerSpec), findPiece(targetSpec)) match {
          ...
          case (Some(attacker), Some(target)) =>
            failUnless(
              attacker.side == side,
              "Attacker is controlled by other side"
            )
            failUnless(target.side != side, "Target piece is friendly")
            failUnless(
              tiles.inBounds(target.loc),
              "Target location not in bounds"
            )
            ...
            failUnless(
              topology.distance(attacker.loc, target.loc) <= attackRange,
      ...
      case Spawn(spawnLoc, spawnName) =>
        ...
        failUnless(
          ...
          "No non-newly-spawned piece with spawn in range"
        )
        failUnless(
          reinforcements(side).contains(spawnName),
          "No such piece in reinforcements"
        )
        ...
        findPiece(pieceSpec) match {
          ...
          case Some(piece) =>
            ...
            failIf(
              ...
              "Piece has already acted or cannot act this turn"
            )
            requireSuccess(ability.tryIsUsableNow(piece))
            failIf(
              ability.isSorcery && mana + manaInHand(side, externalInfo) <= 0,
              "No mana (must first play cantrip or discard spell)"
            )
            ability match {
              case Suicide | SpawnZombies | NecroPickAbility |
                  (_: SelfEnchantAbility) =>
                ()
              case KillAdjacent =>
                failIf(
              ...
              case MoveTerrain | MoveEarthquake | MoveFlood | MoveWhirlwind |
                  MoveFirestorm =>
                failUnless(
                  topology.distance(piece.loc, targets.loc0) <= 1,
                  "Must place terrain in an adjacent hex"
                )
              ...
              case (ability: TargetedAbility) =>
                findPiece(targets.target0) match {
                  ...
                  case Some(target) =>
                    requireSuccess(ability.tryCanTarget(piece, target))
                }
            }
        }
      ...
      case Teleport(pieceSpec, src, dest) =>
        ...
        failUnless(
          tiles(src).terrain == Teleporter,
          "Must be at a teleporter to teleport"
        )
        ...
        findPiece(pieceSpec) match {
          ...
          case Some(piece) =>
            ...
            failUnless(
              piece.loc == src,
              "Teleporting piece is not on the teleporter"
            )
            piece.actState match {
              case Moving(stepsUsed) =>
                failUnless(
                  stepsUsed == 0,
                  "Piece must start turn on teleporter without moving or acting"
                )
              case Attacking(_) | Spawning | DoneActing =>
                fail(
                  "Piece must start turn on teleporter without moving or acting"
            ...
            externalInfo.spellsRevealed.get(spellId) match {
              ...
              case Some(spellName) =>
                Spells.spellMap.get(spellName) match {
                  ...
                  case Some(spell) =>
                    val manaAfter = mana + manaInHand(
                      side,
                      externalInfo,
                      excludingSpell = Some(spellId)
                    ) + manaOfPlay(spell.spellType)
  ...
  private def doActionSingle(
      action: PlayerAction,
      externalInfo: ExternalInfo
  ): Try[Unit] = {
    tryLegalitySingle(action, externalInfo).map { case () =>
      doActionSingleAssumeLegal(action, externalInfo)
    }
  }
  ...
  private def doActionSingleAssumeLegal(
      action: PlayerAction,
      externalInfo: ExternalInfo
  ): Unit = {
    action match {
      ...
      case Attack(attackerSpec, targetSpec) =>
        ...
        attacker.attackedPiecesThisTurn =
          attacker.attackedPiecesThisTurn :+ target
      ...
      case ActivateTile(loc) =>
        tiles(loc).terrain match {
          ...
          case Spawner(spawnName) =>
            ...
            spawnPieceInternal(
              side,
              externalInfo.pieceMap(spawnName),
              loc
            ) match {
              case Some(_: Piece) => ()
              case None           => assertUnreachable()
      ...
      case ActivateAbility(pieceSpec, ability, targets) =>
        ...
        ability match {
          ...
          case NecroPickAbility =>
            ...
            val necroIdx =
              allowedNecros(piece.side).indexOf(piece.baseStats.name)
            val newNecroName = allowedNecros(piece.side)(
              (necroIdx + 1) % allowedNecros(piece.side).size
            )
            ...
            spawnPieceInitial(piece.side, newNecroStats, piece.loc) match {
              case Success(newNecro: Piece) =>
                val necroPick =
                  PieceModWithDuration(PieceMods.NecroPick, turnsLeft = None)
                newNecro.modsWithDuration =
                  newNecro.modsWithDuration :+ necroPick
              case Failure(_) => assertUnreachable()
            }
          case KillAdjacent =>
            pieces.topology.forEachAdj(piece.loc) { loc =>
      ...
      case PlaySpell(spellId, targets) =>
        ...
        spellsPlayed =
          spellsPlayed :+ SpellPlayedInfo(spellId, side, Some(targets))
  ...
  private def killIfEnoughDamage(
      piece: Piece,
      externalInfo: ExternalInfo
  ): Unit = {
    val stats = piece.curStats(this)
  ...
  private def updateAfterPieceKill(
      pieceSide: Side,
      pieceStats: PieceStats,
      loc: Loc,
      externalInfo: ExternalInfo
  ...
  private def killPiece(piece: Piece, externalInfo: ExternalInfo): Unit = {
    if (piece.curStats(this).isSoulbound) {
      ...
      killedThisTurn = killedThisTurn :+ ((
        piece.spec,
        piece.baseStats.name,
        piece.side,
        piece.loc
      ))
      updateAfterPieceKill(
        piece.side,
        piece.curStats(this),
        piece.loc,
        externalInfo
  ...
  private def removeFromBoard(piece: Piece): Unit = {
    ...
    piece.spawnedThisTurn.foreach { spawnedThisTurn =>
      piecesSpawnedThisTurn = piecesSpawnedThisTurn - spawnedThisTurn
    }
  }

  private def addReinforcementInternal(
      side: Side,
      pieceName: PieceName
  ): Unit = {
    reinforcements(side) = reinforcements(side) + (pieceName -> (reinforcements(
      side
    ).getOrElse(pieceName, 0) + 1))
  }

  // Does check for legality of spawn, returning the piece on success
  def spawnPieceInternal(
      spawnSide: Side,
      spawnStats: PieceStats,
      spawnLoc: Loc
  ): Option[Piece] = {
    if (!spawnIsLegal(spawnSide, spawnStats, spawnLoc))
      ...
      val piece = Piece.createInternal(
        spawnSide,
        spawnStats,
        nextPieceId,
        spawnLoc,
