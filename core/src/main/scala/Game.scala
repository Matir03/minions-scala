...
/** TechLevel: How far long the tech tree a side is for a given unit
  */
...
/** Tech: Element in the tech sequence.
  */
sealed trait Tech {
...
/** TechState: State of a single tech.
  */
...
//server->client only
case class PayForReinforcement(side: Side, pieceName: PieceName)
    extends GameAction
case class UnpayForReinforcement(side: Side, pieceName: PieceName)
    extends GameAction
case class ChooseSpell(side: Side, spellId: SpellId, boardIdx: Int)
    extends GameAction
case class UnchooseSpell(side: Side, spellId: SpellId, boardIdx: Int)
    extends GameAction
case class AddWin(side: Side, boardIdx: Int) extends GameAction
case class AddUpcomingSpells(side: Side, spellIds: Array[SpellId])
    extends GameAction
        ...
        if (techState.level(side) == TechAcquired) {
          techState.tech match {
            case PieceTech(pieceName) =>
              piecesAcquired(side) =
                piecesAcquired(side) + (pieceName -> techState)
...
/** Game: The "global" state which isn't local to a board.
  */
case class Game(
    val numBoards: Int,
    var curSide: Side,
  ...
  def tryIsLegal(action: GameAction): Try[Unit] = {
    action match {
      case PayForReinforcement(side, pieceName) =>
        tryCanPayForReinforcement(side, pieceName)
      case UnpayForReinforcement(side, pieceName) =>
        tryCanUnpayForReinforcement(side, pieceName)
      case ChooseSpell(side, spellId, boardIdx) =>
        tryCanChooseSpell(side, spellId, boardIdx)
      case UnchooseSpell(side, spellId, boardIdx) =>
        tryCanUnchooseSpell(side, spellId, boardIdx)
      ...
      case PerformTech(side, techLineIdx) =>
        tryCanPerformTech(side, techLineIdx)
  ...
  def doAction(action: GameAction): Try[Unit] = {
    action match {
      ...
      case AddUpcomingSpells(side, spellIds) => {
        doAddUpcomingSpells(side, spellIds); Success(())
      }
  ...
  private def clearAndDrawSpells(): Unit = {
    ...
    boardsWithSpells = Map()
    while (
      upcomingSpells(curSide).nonEmpty && spellsToChoose.length < numBoards + 1
    ) {
  ...
  def resolveSpellChoice(
      idx: Int,
      side: Option[Side]
  ): (Option[SpellId], Option[Side]) = {
    side match {
      case None => (None, None)
      case Some(ourSide) =>
        if (ourSide == curSide) {
          if (idx == -1) {
            ...
            val side =
              if (spellsChosen.contains(spellId)) None else Some(ourSide)
            (Some(spellId), side)
          } else if (idx - spellsToChoose.length == 0) {
            (None, None) // Upcoming spells label
          } else if (
            idx - spellsToChoose.length - 1 < upcomingSpells(ourSide).length
          ) {
            (
              Some(upcomingSpells(ourSide)(idx - spellsToChoose.length - 1)),
              None
            )
          } else {
  ...
  def endTurn(): Unit = {
    ...
    techLine.foreach { techState =>
      Side.foreach { side =>
        if (
          techState.startingLevelThisTurn(side) != TechAcquired && techState
            .level(side) == TechAcquired
        ) {
  ...
  private def tryCanPayForReinforcement(
      side: Side,
      pieceName: PieceName
  ): Try[Unit] = {
    if (side != curSide)
      ...
      Failure(
        new Exception(
          "Trying to pay for reinforcement piece with unknown name: " + pieceName
        )
      )
    else {
      piecesAcquired(side).get(pieceName) match {
        ...
        case Some(techState) =>
          if (techState.startingLevelThisTurn(side) != TechAcquired)
            Failure(
              new Exception(
                "Cannot buy pieces on the same turn as teching to them"
              )
            )
          else {
  ...
  private def payForReinforcement(
      side: Side,
      pieceName: PieceName
  ): Try[Unit] = {
  ...
  private def tryCanUnpayForReinforcement(
      side: Side,
      pieceName: PieceName
  ): Try[Unit] = {
    if (side != curSide)
      ...
      Failure(
        new Exception(
          "Trying to unpay for reinforcement piece with unknown name: " + pieceName
        )
      )
    else
      Success(())
  ...
  private def unpayForReinforcement(
      side: Side,
      pieceName: PieceName
  ): Try[Unit] = {
  ...
  private def tryCanChooseSpell(
    ...
    if (side != curSide)
      ...
      if (
        boardsWithSpells.contains(boardIdx) && !startedWithTech(Metamagic, side)
      )
        Failure(
          new Exception("This board has already chosen a spell this turn")
        )
      else if (spellsChosen.size == numBoards)
        Failure(
          new Exception(
            "Cannot choose more than " + numBoards + " spells in one turn"
          )
        )
      else
        Success(())
  ...
  private def chooseSpell(
    ...
    tryCanChooseSpell(side, spellId, boardIdx) match {
      ...
      case (suc: Success[Unit]) =>
        ...
        boardsWithSpells = boardsWithSpells + (boardIdx -> (1 + boardsWithSpells
          .getOrElse(boardIdx, 0)))
        suc
    }
  }

  private def tryCanUnchooseSpell(
      side: Side,
      spellId: SpellId,
      boardIdx: Int
  ): Try[Unit] = {
  ...
  private def doAddUpcomingSpells(
      side: Side,
  ...
  private def tryCanPerformTech(side: Side, techLineIdx: Int): Try[Unit] = {
    if (side != curSide)
      ...
      Failure(
        new Exception("Must buy extra tech to tech additional times this turn")
      )
    else if (techLineIdx < 0 || techLineIdx >= techLine.length)
      Failure(new Exception("Invalid tech idx"))
    else if (
      techLineIdx > 0 && techLine(techLineIdx - 1).level(side) == TechLocked
    )
      ...
      (techState.level(side), techState.level(side.opp)) match {
        case (TechAcquired, _) =>
          Failure(new Exception("Already own this tech"))
  ...
  private def tryCanUndoTech(side: Side, techLineIdx: Int): Try[Unit] = {
    if (side != curSide)
      ...
      techState.level(side) match {
        case TechLocked =>
          Failure(new Exception("Cannot undo tech never acquired"))
        case TechUnlocked | TechAcquired =>
          if (
            techLine(techLineIdx).level(side) == techLine(techLineIdx)
              .startingLevelThisTurn(side)
          )
            Failure(new Exception("Cannot undo tech from previous turns"))
          else if (
            techLineIdx < techLine.length - 1 &&
            techLine(techLineIdx).level(side) == TechUnlocked &&
            techLine(techLineIdx + 1).level(side) != techLine(techLineIdx + 1)
              .startingLevelThisTurn(side)
          )
            Failure(
              new Exception(
                "Cannot undo this tech without undoing later techs first"
              )
            )
          else
            Success(())
            ...
            techState.tech match {
              ...
              case Copycat =>
                techLine.zipWithIndex.foreach { case (state, idx) =>
                  if (
                    state.startingLevelThisTurn(side) != state.level(side) &&
  ...
  private def tryCanUnbuyExtraTechAndSpell(side: Side): Try[Unit] = {
    if (side != curSide)
      ...
      Failure(
        new Exception(
          "Undo not possible since extra spell was claimed, that board must first undo claiming that spell"
        )
      )
    else if (usedTechsThisTurn >= techsThisTurn())
      Failure(
        new Exception(
          "Undo not possible since extra tech was used, must first choose a tech to undo"
        )
      )
    else
      Success(())
