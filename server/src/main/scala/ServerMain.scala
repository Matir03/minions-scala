...
object ServerMain extends App {
  ...
  case class UserJoined(
      ...
      val out: ActorRef
  ) extends GameActorEvent
  case class UserLeft(val sessionId: Int) extends GameActorEvent
  case class QueryStr(val sessionId: Int, val queryStr: String)
      extends GameActorEvent
  ...
  private class GameActor(state: GameState, gameid: String) extends Actor {
    ...
    private def scheduleEndOfTurn(reason: ScheduleReason): Unit = {
      ...
      if (!state.isPaused) {
        ...
        endTurnJob = Some(
          actorSystem.scheduler.scheduleOnce(turnTimeLeft seconds) {
    ...
    override def receive: Receive = {
      ...
      case QueryStr(sessionId, queryStr) =>
        if (state.usernameOfSession.contains(sessionId)) {
          ...
          Try(Json.parse(queryStr)) match {
            case Failure(err) =>
              out ! Protocol.QueryError(
                "Could not parse as json: " + err.getLocalizedMessage
              )
            case Success(json) =>
      ...
      case ShouldEndTurn(turnNumberToEnd) =>
        if (
          state.game.turnNumber == turnNumberToEnd && state.game.winner.isEmpty
        ) {
  ...
  def websocketMessageFlow(
      gameid: String,
      username: String,
      sideStr: Option[String]
    ...
    val in: Sink[Message, _] = {
      Flow[Message]
        ...
        .to(
          Sink.actorRef[GameActorEvent](
            gameActor,
            onCompleteMessage = UserLeft(sessionId)
          )
        )
    }
    ...
    val out: Source[Message, _] = {
      Source
        .actorRef[Protocol.Response](responseBufferSize, OverflowStrategy.fail)
        .mapMaterializedValue(actorRef =>
          gameActor ! UserJoined(sessionId, username, side, actorRef)
        )
  ...
  def addAI(gameState: GameState, gameid: String, doTutorial: Boolean) = {
    ...
    val ai = actorSystem.actorOf(
      Props(classOf[AIActor], actorRef, gameState, doTutorial)
    )
  ...
  def addSpookyAI(gameState: GameState, gameid: String, enginePath: String) = {
    ...
    val source = Source.fromPublisher(pub)
    val ai =
      actorSystem.actorOf(SpookyAI.props(actorRef, gameState, enginePath))
    ...
    val sink: Sink[Message, _] = {
      Flow[Message]
        ...
        .to(
          Sink.actorRef[Protocol.Response](
            ai,
            onCompleteMessage = Protocol.UserLeft("spooky", Some(S1))
          )
        )
    }
                        ...
                        games.get(gameid) match {
                          ...
                          case Some((_, state)) =>
                            (password, state.password) match {
                              case (None, Some(_)) =>
                                complete(
                                  gameid + " is password-protected; please provide 'password=' in URL"
                                )
    ...
    path("newGame") {
      get {
        ...
        val vacuum_html = {
          ...
          val bools =
            List("Swarm", "Lumbering", "Spawn", "Persistent", "Flying", "Blink")
          val text_html =
            attrs.map { attr =>
              s"""<div class="field">
            <label>$attr</label>
          ...
          val ability_options = Abilities.abilityMap.keys
            .map { name =>
        ...
        formFields(('game, 'password, 'seed)) { (gameid, password, seed) =>
          formFields(
            (
              'blueSeconds.as[Double],
              'redSeconds.as[Double],
              'targetWins.as[Int]
            )
          ) { (blueSeconds, redSeconds, targetWins) =>
            formFields(
              (
                'blueSouls.as[Int],
                'redSouls.as[Int],
            ...
            ) { (blueSouls, redSouls, techSouls, maps) =>
              formFields(
                ('blueSoulsPerTurn.as[Int], 'redSoulsPerTurn.as[Int])
              ) { (blueSoulsPerTurn, redSoulsPerTurn) =>
                formFields(
                  ('blueName, 'redName, 'blueAttack, 'redAttack, 'blueHealth,
                    'redHealth, 'blueSpeed, 'redSpeed, 'blueRange, 'redRange,
                    'blueCost, 'redCost, 'blueRebate, 'redRebate,
                    'blueNumAttacks, 'redNumAttacks, 'blueAbility, 'redAbility)
                ) {
                  (
                      blueName,
                      redName,
                    ...
                    formFields(
                      ...
                      (
                        ...
                        games.get(gameid) match {
                          case Some(_) =>
                            complete(
                              s"""A game named "$gameid" already exists; pick a different name"""
                            )
                          case None =>
                            val seed_opt =
                              if (seed == "") None else Some(seed.toLong)
                            val maps_opt =
                              if (maps.isEmpty) None else Some(maps.toList)
                            val passwordOpt =
                              if (password == "") None else Some(password)
                            val startingSouls =
                              SideArray.createTwo(blueSouls, redSouls)
                            val secondsPerTurn =
                              SideArray.createTwo(blueSeconds, redSeconds)
                            val extraSoulsPerTurn = SideArray.createTwo(
                              blueSoulsPerTurn,
                              redSoulsPerTurn
                            )

                            val blueUnit: Option[PieceStats] = Units.fromForm(
                              blueName,
                              blueAttack,
                              blueHealth,
                              blueSpeed,
                            ...
                            val redUnit: Option[PieceStats] = Units.fromForm(
                              redName,
                              redAttack,
                              redHealth,
                              redSpeed,
